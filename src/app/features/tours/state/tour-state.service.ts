...
export class TourStateService {
  ...
  addLog(tourId: string, log: TourLog): void {
    const tour = this.tourService.tours().find(t => t.id === tourId);
    if (!tour) return;
    this.tourService.update({ ...tour, logs: [...tour.logs, log] });
  }

  updateLog(tourId: string, log: TourLog): void {
    const tour = this.tourService.tours().find(t => t.id === tourId);
    if (!tour) return;
    this.tourService.update({ ...tour, logs: tour.logs.map(l => l.id === log.id ? log : l) });
  }

  deleteLog(tourId: string, logId: string): void {
    const tour = this.tourService.tours().find(t => t.id === tourId);
    if (!tour) return;
    this.tourService.update({ ...tour, logs: tour.logs.filter(l => l.id !== logId) });
  }
